...
const ADZUNA_API_KEY = process.env.ADZUNA_API_KEY as string;
...
async function getPDFMetadata(filePath: string): Promise<any> {
  try {
    const pdf = await getPdfParse();
    const dataBuffer = await fs.readFile(filePath);
    const data = await pdf(dataBuffer);
...
async function searchJobs(query: string, location: string = "Zurich, Switzerland"): Promise<any[]> {
  try {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    switch (name) {
      ...
      case "read_pdf": {
        const filePath = path.isAbsolute((args as any).file_path)
          ? (args as any).file_path
          : path.join(WATCHED_DIRECTORY, (args as any).file_path);
      ...
      case "search_pdfs": {
        ...
        const matches = pdfFiles.filter(f => 
          path.basename(f).toLowerCase().includes(query)
        );
      ...
      case "answer_career_question": {
        const answer = await answerQuestion((args as any).question);
      ...
      case "search_zurich_jobs": {
        const jobs = await searchJobs(
          (args as any).query,
          (args as any).location || "Zurich, Switzerland"
        );
          ...
          content: [
            {
              ...
              text: JSON.stringify({
                query: (args as any).query,
                location: (args as any).location || "Zurich, Switzerland",
                count: jobs.length,
